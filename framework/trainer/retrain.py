...
from .base_backup import KGTrainer
...
class RetrainTrainer(Trainer):
    ...
    def freeze_unused_mask(self, model, edge_to_delete, subgraph, h):
        ...
        for s, t in edges:
            if s < t:
                gradient_mask[s, t] = 1
        gradient_mask = gradient_mask.to(self.args.device)
    ...
    def train_fullbatch(self, model, data, optimizer, args, logits_ori=None, attack_model_all=None, attack_model_sub=None):
        ...
        for epoch in trange(args.epochs, desc='Unlearning'):
            ...
            z = model(data.x, data.train_pos_edge_index[:, data.dr_mask])  # torch.Size([N, 64])
            logits = model.decode(z, data.train_pos_edge_index[:, data.dr_mask], neg_edge_index)  # torch.Size([N, ])
            ...
            if (epoch + 1) % self.args.valid_freq == 0:
                valid_loss, dt_auc, dt_aup, df_auc, df_aup, df_con_auc, df_con_mse, df_logit, logit_all_pair, valid_log = self.eval(model, data, 'val')

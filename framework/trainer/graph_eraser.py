...
class ConstrainedKmeans:
    ...
    def generate_shard_data(self, data):
        ...
        for shard in trange(self.args['num_shards'], desc='Generate shard data'):
            ...
            shard_indices = np.union1d(train_shard_indices, self.eval_GraphEraser_indices)
...
class Aggregator:
    ...
    def aggregate(self, aggregator='mean'):
        if aggregator == 'mean':
            aggregate_f1_score = self._mean_aggregator()
        elif aggregator == 'optimal':
...
class GraphEraserTrainer(Trainer):
    def train(self, model, data, optimizer, args, logits_ori=None, attack_model_all=None, attack_model_sub=None):
        ...
        preprocess_time_cost = preprocess_time - start_time

        shard_time_cost_list = []

        for shard_id in range(args.num_clusters):
            shard_time_start = time.time()
            print('\n')
            print(f'Shard {shard_id} is retraining.')
            ...
            subset_train, _ = subgraph(
                torch.tensor(community[shard_id], dtype=torch.long, device=self.args.device),
                data.train_pos_edge_index,
                num_nodes=data.num_nodes)
            self.subsets['shard_id'] = subset_train

            self.train_model(model_shard_id, data, subset_train, optimizer, args, shard_id)

            with torch.no_grad():
                z = model_shard_id(data.x, subset_train)
                logits = model_shard_id.decode(z, data.test_pos_edge_index, data.test_neg_edge_index)

            shard_time_end = time.time()
            shard_time_cost = shard_time_end - shard_time_start
            shard_time_cost_list.append(shard_time_cost)



        self.trainer_log['para_training_time_cost'] = preprocess_time_cost + max(shard_time_cost_list)

        end_time = time.time()
        cost_time = end_time - start_time
        self.trainer_log['seq_training_time_cost'] = cost_time
        print()
        # weight_para = nn.Parameter(torch.full((self.num_shards,), fill_value=1.0 / self.num_shards), requires_grad=True)
        # optimizer = optim.Adam([weight_para], lr=self.args.lr)


        # aggregator = Aggregator(0, model, data, community, args)
    ...
    def train_model(self, model, data, subset_train, optimizer, args, shard_id):
        ...
        valid_loss, auc, aup, df_logit, logit_all_pair  = self.eval_model(model, data, subset_train, 'val')
        log = {
    ...
    def eval_model(self, model, data, subset_train, stage='val', pred_all=False):
        ...
        if self.args.unlearning_model in ['original', 'retrain']:
            ...
            df_logit = float('nan')
            # df_logit = model.decode(z, subset_train).sigmoid().detach().cpu().tolist()
    ...
    @torch.no_grad()
    def eval_aggregated_model(self, model, data, subset_train, stage='val', pred_all=False):
        model.eval()
        pos_edge_index = data[f'{stage}_pos_edge_index']
        neg_edge_index = data[f'{stage}_neg_edge_index']
